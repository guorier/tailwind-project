...
- Codex Luna owns the first bounded implementation pass and the final verification pass.
- Codex is available through the project MCP server as `codex`.
- For the first pass, invoke `luna_worker` with `gpt-5.6-luna`, `model_reasoning_effort = "max"`, and `workspace-write`; give it an explicit file scope and require an uncommitted bounded change.
- Claude reviews Luna's diff and owns readability, naming, consistency, security hardening, and final polish edits.
- After Claude's polish, invoke Luna again with `gpt-5.6-luna`, `model_reasoning_effort = "max"`, and `read-only` to verify behavior and report findings.
- Do not let Claude and Codex edit the same files concurrently.
- Require evidence from every Luna pass: paths, symbols, commands, results, and remaining risks.
