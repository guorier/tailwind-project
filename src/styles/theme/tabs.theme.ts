...
export const tabTheme: TabsTheme = {
  ...
  tablist: {
    ...
    styles: {
      ...
      underline: "flex-wrap -mb-px border-b border-gray-200 text-primary-500",
      pills: "flex-wrap font-medium text-sm text-primary-500 space-x-2",
      fullWidth:
        "w-full text-sm font-medium divide-x divide-gray-200 shadow grid grid-flow-col rounded-none",
    },
    tabitem: {
      // 배경색은 표시 방식별 styles 에서 지정한다. 공통 base 에 두면 twMerge 병합 순서상
      // 배경을 지정하지 않는 underline·default 비활성 탭까지 덮어써 버린다.
      base: "flex items-center justify-center p-4 rounded-t-lg text-sm font-medium first:ml-0 disabled:cursor-default disabled:text-silver-400 focus:ring-primary-500 focus:outline-none",
      styles: {
        default: {
          base: "rounded-t-lg",
          active: {
            on: "bg-silver-100 text-primary-600",
        ...
        underline: {
          base: "rounded-t-lg",
          active: {
            on: "text-primary-600 rounded-t-lg border-b-2 border-primary-600 active",
  ...
  tabitemcontainer: {
    ...
    styles: {
      ...
      pills: "text-primary-600",
      fullWidth: "text-primary-600",
    },
  },
  tabpanel: "py-3",
};
