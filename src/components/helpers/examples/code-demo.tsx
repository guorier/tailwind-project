...
export function CodeDemo({ data }: CodeDemoProps) {
  ...
  return (
    <div className="code-example mt-8">
      <div className="w-full rounded-t-xl border border-gray-200 bg-silver-50 p-4 dark:border-gray-600 dark:bg-silver-700">
        <div className="grid grid-cols-2">
          <div className="mr-auto flex justify-center items-center font-bold text-lg">
            {data.title}
          </div>
      ...
      <div className="code-syntax-wrapper">
        ...
        >
          <div className="flex w-full rounded-t-md border-b border-gray-200 bg-silver-50 dark:border-gray-600 dark:bg-silver-700">
          ...
          <div className={twMerge('!overflow-y-hidden', shouldExpand && !isExpanded && 'max-h-4')}>
...
function Tabs({ tabIndex, items, onSelect }: { tabIndex: number; items: CodeItem[]; onSelect(index: number): void }) {
  return (
    <ul className="flex flex-1 text-center text-sm font-medium text-silver-500 dark:text-silver-400">
      {items.map((item, index) => (
        <li key={item.fileName}>
          <button type="button" role="tab" onClick={() => onSelect(index)}>
            <span
              className={twMerge(
                'inline-block w-full border-r border-gray-200 bg-silver-100 p-2 px-3 dark:border-gray-600 dark:bg-silver-800',
                index === tabIndex ? 'text-silver-800 dark:text-white' : 'hover:text-silver-600 hover:dark:text-silver-300',
              )}
            >
...
function CodePreview({ isDarkMode, children }: PropsWithChildren<{ isDarkMode: boolean }>) {
  return (
    <div className={twMerge('code-preview-wrapper', isDarkMode && 'dark')}>
      <div className="code-preview flex border-x border-gray-200 bg-white bg-gradient-to-r p-0 dark:border-gray-600 dark:bg-silver-900">
        <div className="code-responsive-wrapper w-full">
          <div className="mx-auto w-full bg-white bg-gradient-to-r p-2 dark:bg-silver-900 sm:p-6">
...
function EditOnGithubButton({ githubSlug }: { githubSlug: string }) {
  ...
  return (
    <a
      ...
      className="inline-flex w-fit items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-center text-xs font-medium text-silver-900 hover:bg-silver-100 hover:text-primary-700 focus:text-primary-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:border-gray-600 dark:bg-silver-800 dark:text-silver-400 dark:hover:bg-silver-700 dark:hover:text-white dark:focus:text-white dark:focus:ring-gray-700"
    >
      <svg
        aria-hidden
        fill="currentColor"
...
function ToggleDarkModeButton({ isDarkMode, onClick }: ComponentProps<'button'> & { isDarkMode: boolean }) {
  return (
    <Tooltip content={isDarkMode ? 'Toggle light mode' : 'Toggle dark mode'}>
      <button
        onClick={onClick}
        className="flex items-center rounded-lg border border-gray-200 bg-white p-2 text-xs font-medium text-silver-700 hover:bg-silver-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-2 focus:ring-gray-300 dark:border-gray-600 dark:bg-silver-800 dark:text-silver-400 dark:hover:bg-silver-700 dark:hover:text-white dark:focus:ring-gray-500"
      >
...
function CopyToClipboardButton({ isJustCopied, onClick }: ComponentProps<'button'> & { isJustCopied: boolean }) {
  return (
    <button
      onClick={onClick}
      className="copy-to-clipboard-button flex items-center border-l border-gray-200 bg-silver-100 px-3 py-2 text-xs font-medium text-silver-600 hover:text-primary-700 dark:border-gray-600 dark:bg-silver-800 dark:text-silver-400 dark:hover:text-white"
    >
      {isJustCopied ? (
...
function CollapseExpandButton({ isExpanded, onClick }: ComponentProps<'button'> & { isExpanded: boolean }) {
  return (
    <button
      onClick={onClick}
      className="absolute bottom-0 left-0 z-1 w-full border-t border-gray-200 bg-silver-100 px-5 py-2.5 text-sm font-medium text-silver-900 hover:bg-silver-100 hover:text-primary-700 dark:border-gray-600 dark:bg-silver-700 dark:text-silver-400 dark:hover:bg-silver-700 dark:hover:text-white"
    >
